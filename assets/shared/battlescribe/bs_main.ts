...
import type { Translated } from "./bs_translation";
...
export class Base {
  ...
  forceEntries?: Force[];
  sharedForceEntries?: Force[];
  forceEntryLinks?: ForceEntryLink[];
  ...
  isForce(): this is Force | ForceEntryLink {
    return false;
  }
  ...
  isCharacteristic(): this is Characteristic {
    return false;
  }
  ...
  getRawTypeName(): string | undefined {
    return (this as unknown as Translated<typeof this>).original_typeName ?? this.typeName;
  }
  ...
  *forcesIteratorRecursive(): Iterable<Force | ForceEntryLink> {
    ...
    if (this.forceEntryLinks) {
      for (const force of this.forceEntryLinks) {
        yield force;
        yield* force.forcesIteratorRecursive();
      }
    }
  ...
  *infoProfilesIterator(): Iterable<Profile> {
  ...
  forEachCond(callbackfn: (value: Base | Link, depth: number) => any, _depth = 0): void {
    if (callbackfn(this, 0) === undefined) for (const instance of this.selectionsIterator()) instance.forEachCond(callbackfn, _depth + 1);
  }
...
export class Link<T extends Base = Group | Entry> extends Base {
  ...
  isExportable(): boolean {
    return this.exportable ?? this.target.isExportable();
  }
  isCollapsible(): boolean | undefined {
  ...
  getDefaultSelectionEntryId() {
    return (this as any as Group).defaultSelectionEntryId || (this.target as any as Group)?.getDefaultSelectionEntryId();
  }
  getCategoryEntryId(): string | undefined {
  ...
  *infoProfilesIterator(): Iterable<Profile> {
  ...
  getName(): string {
    return String(this.target?.name ?? this.name ?? "");
  }
  getPrimaryCategory(): string {
    for (const categoryLink of this.categoryLinks || []) {
      if (categoryLink.primary) return categoryLink.targetId;
...
export class InfoLink<T extends Rule | InfoGroup | Profile = Rule | InfoGroup | Profile> extends Link {
  ...
  getRawTypeName() {
    return this.target?.getRawTypeName()
  }
...
export class ForceEntryLink extends Link {
  declare target: Force;
  declare type: "force";
  declare id: string;
  ...
  forces?: Force[];
  main_catalogue!: Catalogue;
  isForce(): this is ForceEntryLink {
    return true;
  }
  getChildForcesLabel() {
    return this.target?.childForcesLabel ?? this.childForcesLabel ?? "Forces";
  }
  *selectionsIterator(): Iterable<Base> {
...
export class Force extends Base {
  ...
  getChildForcesLabel() {
    return this.childForcesLabel ?? "Forces";
  }

  *infoLinksIterator(): Iterable<InfoLink> {
    if (this.isForce()) {
      if (this.main_catalogue) {
        for (const imported of this.main_catalogue.imports) {
          yield* imported.infoLinksIterator();
        }
        yield* this.main_catalogue.infoLinksIterator();
      }
    }
    yield* super.infoLinksIterator();
  }
  ...
  *infoRulesIterator(): Iterable<Rule> {
    if (this.isForce()) {
      if (this.main_catalogue) {
        ...
        yield* this.main_catalogue.infoRulesIterator();
      }
    }
    yield* super.infoRulesIterator();
  }
...
export function getTheoreticalMaxes(constraints: Iterable<BSIConstraint>, modifierGroups: Iterable<BSIModifierGroup>): number[] {
...
export class Characteristic extends Base implements BSICharacteristic {
  declare type: BSICharacteristicType;
  declare typeId: string;
  ...
  getLabel() {
    if (this.catalogue) {
      const ct = (this.catalogue.findOptionById(this.typeId) as Base & BSICharacteristicType)
      return ct?.name ?? this.typeId;
    }
    return this.typeId;
  }
  getTypeName() {
  ...
  getRawTypeName() {
    if (this.catalogue) {
      const ct = (this.catalogue.findOptionById(this.typeId) as Base & BSICharacteristicType)
      return (ct as unknown as Translated<typeof ct>)?.original_name ?? ct?.name ?? this.typeId;
    }
    return this.typeId;
  }
  getName() {
...
export function* iterateModifierGroupsRecursive(groups?: Iterable<BSIModifierGroup>): Generator<BSIModifierGroup, void, undefined> {
  if (groups) {
...
export function entriesToJson(data: Array<Base | Record<string, any>> | Base | Record<string, any>, extraFields?: Set<string>, options?: EntriesToJsonOptions): string {
...
export function forEachObjectWhitelist2<T extends Base>(current: Base, callbackfn: (value: T, parent: T) => unknown, whiteList = arrayKeys) {
  for (const key in current) {
    if (whiteList.has(key)) {
...
export function deconstruct_affects_query(query?: string): AffectsQuery {
  ...
  const split = (query || "").replace("childs", "entries").split(".");
