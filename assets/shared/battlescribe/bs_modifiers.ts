...
import { Condition, Modifier, ModifierGroup, Link, Constraint, Base, ConditionGroup, deconstruct_affects_query, readable_affects_query } from "./bs_main";
...
export function modifierToString(
  ...
  const relative = []
  if (modifier.scope) relative.push(modifier.scope);
  if (modifier.affects) relative.push(readable_affects_query(deconstruct_affects_query(modifier.affects), (id) => fieldToString(base, id)));
  const _in = relative.length ? ` in ${relative.join('.')}` : ""
