...
import { getAllConditions, getModifierOrConditionParent } from "./bs_modifiers";
...
export class Catalogue extends Base {
  ...
  forces!: Array<Force | ForceEntryLink>;
  categories!: Category[];
  ...
  processForEditor() {
    ...
    forEachObjectWhitelist2<EditorBase>(
      this,
      (cur) => (cur.isLink() ? this.updateLink(cur) : this.refreshErrors(cur)),
      arrayKeys
    );
  }
  get url(): string {
  ...
  *forceEntriesIterator(): Iterable<Force | ForceEntryLink> {
    for (const catalogue of this.imports) {
      if (catalogue.forceEntries) yield* catalogue.forceEntries
      if (catalogue.forceEntryLinks) yield* catalogue.forceEntryLinks
      if (catalogue.sharedForceEntries) yield* catalogue.sharedForceEntries
    }
    if (this.forceEntries) yield* this.forceEntries;
    if (this.forceEntryLinks) yield* this.forceEntryLinks;
    if (this.sharedForceEntries) yield* this.sharedForceEntries;
  }
  *forcesIterator(): Iterable<Force | ForceEntryLink> {
    for (const catalogue of this.imports) {
      if (catalogue.forceEntries) {
        for (const force of catalogue.forceEntries)
          if (force.import !== false) yield force
      }
      if (catalogue.forceEntryLinks) {
        for (const force of catalogue.forceEntryLinks) {
          if (force.import !== false) yield force
        }
      }
    }
  ...
  *forcesIteratorRecursive(): Iterable<Force | ForceEntryLink> {
    for (const catalogue of this.imports) {
      for (const force of catalogue.forceEntries || []) {
        yield force;
    ...
    if (this.sharedForceEntries) {
      for (const force of this.sharedForceEntries) {
        yield force
        yield* force.forcesIteratorRecursive();
      }
    }
  ...
  *selectionsIterator(): Iterable<Base> {
    ...
    for (const imported of [this, ...this.imports]) {
      for (const val of Object.values(imported.index) as EditorBase[]) {
        ...
        if ((name && String(name).match(regx))) {
          result.push(val);
        } else if (comment && String(comment).match(regx)) {
          result.push(val);
        } else if (id === text) {
          result.push(val)
        }
  ...
  // Mark entries that have no min constraints or reason to have an amount above 0 by default
  // Groups that contain only lazy nodes are also lazy
  markLazyNodes(startNode: Base | Link) {
    function hasNonSelfCondition(node: Base | Link, group: BSIModifierGroup) {
      for (const condition of getAllConditions(group)) {
        if (condition.scope !== "self" && condition.childId !== node.id) return true
      }
      return false
    }
    function hasPositiveMinConstraint(node: Base | Link) {
      if (node.getDefaultAmount()) return true
      let hasMin = false;
      let constraintIds = new Set<string>();
      for (const constraint of node.constraintsIterator()) {
        if (constraint.scope === "roster") continue;
        if (constraint.scope === "force") continue;

        if (constraint.type === "min" || constraint.type === "exactly") {
          if (constraint.value > 0) return true;
          constraintIds.add(constraint.id);
          hasMin = true;
        }
      }
      // check for modifiers that could increment min
      if (hasMin) {
        for (const modifier_group of node.modifierGroupsIteratorRecursive()) {
          for (const modifier of modifier_group.modifiers || []) {
            if (!constraintIds.has(modifier.field)) continue;

            if (modifier.type === "increment" || modifier.type === "set" && Number(modifier.value) > 0) {
              if (hasNonSelfCondition(node, modifier_group)) return true
            }
          }
        }
      }
    ...
    function step(node: Base | Link, depth = 0): boolean {
      if (node.lazy !== undefined) return node.lazy;
      let child_count = 0;
      let child_count_lazy = 0

      for (const entry of node.entriesIterator()) {
        if (step(entry, depth + 1)) child_count_lazy++;
        child_count++;
      }
      if (node.isGroup()) {
        if (child_count === child_count_lazy && !hasPositiveMinConstraint(node) && !node.flatten) {
          node.lazy = true;
        } else {
          node.lazy = false;
        }
      } else if (node.isEntry()) {
        if (!hasPositiveMinConstraint(node) && !node.getDefaultAmount()) {
          node.lazy = true;
        } else {
          node.lazy = false;
        }
  ...
  generateForces(categories: Record<string, Category>, forces: Iterable<Force | ForceEntryLink>, parent?: Force | ForceEntryLink | Catalogue): Array<Force | ForceEntryLink> {
    ...
    for (const force of forces) {
      ...
      for (const link of force.categoryLinksIterator()) {
    ...
    sortByAscendingInplace(result, (o) => o.sortIndex ?? 1000)
    parent.forces = result;
    parent.childs = result;
    return result;
  }
  generateCategory(category: Category, units: Record<string, Base[]>) {
    const copied = clone(category);
    copied.main_catalogue = this;
  ...
  generateCategories(units: Record<string, Base[]>): Record<string, Category> {
    ...
    for (const category of this.iterateCategoryEntries()) {
      const copied = this.generateCategory(category, units)
  ...
  generateUnits(): Record<string, Base[]> {
    this.units = sortByAscendingInplace([...this.iterateAllRootEntries()], (o) => o.getName());
    return groupBy(this.units, (o) => o.getPrimaryCategory());;
  }
  generateExtraConstraints() {
    ...
    function localAddBoundCategoryConstraints(
      catalogue: Catalogue,
      category: Category | Force | ForceEntryLink,
    ...
    const recursiveForces = [...this.forces]
    for (const force of recursiveForces) {
      if (force.forces) recursiveForces.push(...force.forces)
    ...
    for (const force of recursiveForces) {
      ...
      if (force.isLink() && force.targetId in by_id_constraints) {
        force_extra_constraints.push(...by_id_constraints[force.targetId]);
      }
      for (const category of force.categories) {
      ...
      for (const nested_force of force.forcesIterator()) {
        for (const constraint of nested_force.constraintsIterator()) {
          if (constraint.type === "min" || constraint.type === "exactly") {
            if (["parent", "force", ...getIds(force)].includes(constraint.scope))
              force_extra_constraints.push(nested_force.getBoundConstraint(constraint));
            else if (["roster"].includes(constraint.scope)) {
              const hash = `${constraint.id}::${nested_force.id}`;
              roster_constraints[hash] = nested_force.getBoundConstraint(constraint)
            }
          }
        }
      }
  ...
  refreshErrors(cur: EditorBase, deleted = false) {
    ...
    const comment = cur.comment === undefined ? "" : String(cur.comment);
    ...
    if (cur.isLink()) {
      ...
      const target = resolveId(cur.targetId, this.getIndexes()) as EditorBase;
  ...
  resolveAllLinks(deleteBadLinks = true) {
    ...
    const unresolvedProfileTypes: Array<Profile> = [];
    const unresolvedCharacteristicTypes: Array<Characteristic> = [];
    ...
    forEachObjectWhitelist2(
      ...
      (cur, parent) => {
        ...
        if (cur.isProfile()) {
          delete (cur as Partial<Profile>).type;
          unresolvedProfileTypes.push(cur as Profile);
        }
        if (cur.isCharacteristic()) {
          delete (cur as Partial<Characteristic>).type;
          unresolvedCharacteristicTypes.push(cur as Characteristic);
        }
    ...
    if (!globalThis.isEditor) {
      resolveTypes(unresolvedProfileTypes, indexes)
      resolveTypes(unresolvedCharacteristicTypes, indexes)
    }
  ...
  getForceLabel(): string {
    return this.gameSystem.getForceLabel();
  }
  getCategoriesLabel(): string {
    return this.gameSystem.getCategoriesLabel();
  }
}

export class GameSystem extends Catalogue {
  forceLabel?: string;
  categoriesLabel?: string;
  getForceLabel(): string {
    return this.forceLabel ?? "Force"
  }
  getCategoriesLabel() {
    return this.categoriesLabel ?? "Categories"
  }
}

export function resolveId(id: string, indexes: Record<string, Base>[]) {
...
export function resolveLinks(
  ...
  while (unresolved.length !== previousLength) {
    ...
    for (let i = 0; i < currentUnresolved.length; i++) {
      ...
      const target = resolveId(id, indexes);
      if (target) {
        if (target.isLink()) {
...
/**
 * Fills in the `.type` field with the value of the first matching key inside indexes
 * Example: indexes: [{A: 1}, {A: 2}] targetId `A` target would result in `1`
 * @param unresolved The types to resolve
 * @param indexes Array of indexes which match an id to a node
 */
export function resolveTypes(
  unresolved: Array<Profile | Characteristic> = [],
  indexes: Record<string, Base>[]
) {
  let previousLength = 0;
  while (unresolved.length !== previousLength) {
