 ...
 * @param callbackfn The function to call with each value
 */
export function forEachPairRecursive(obj: any, callbackfn: (value: any, key: string, obj: Record<string, any>, path?: string[]) => unknown, filter?: (value: any, key: string) => boolean) {
  // Use recursive approach to avoid the path corruption issue with iterative stack
      ...
      if (isObject(value)) {
        ...
        if (Array.isArray(value)) {
          for (const _value of value) {
            ...
            callbackfn(_value, key, current, currentPath);
            traverse(_value, currentPath);
          }
        }
        // Object
        else {
          if (filter && !filter(value, key)) continue;
          callbackfn(value, key, current, currentPath);
...
type ArrayElement<ArrayType extends readonly unknown[]> = ArrayType extends readonly (infer ElementType)[]
  ? ElementType
  : never;
type ArrayPropertyType<O, KT extends keyof O> =
  NonNullable<O[KT]> extends unknown[] ? ArrayElement<NonNullable<O[KT]>> : never;
...
export function recursive<T>(obj: T, callback: (cur: T) => Iterable<T>): T[] {
  const result = [...callback(obj)];
  for (let i = 0; i < result.length; i++) {
    for (const newResult of callback(result[i])) {
