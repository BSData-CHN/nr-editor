export function getRandomKey(): string {
  return Date.now().toString(36).substring(6) + Math.random().toString(36).substring(8);
}
...
export function timeSince(date: Date) {
  ...
  if (interval >= 2) return { amount: interval, unit: $t("time_years") }
  if (interval >= 1) return { amount: interval, unit: $t("time_year") }

  interval = Math.floor(seconds / 2592000); // months
  if (interval >= 2) return { amount: interval, unit: $t("time_months") }
  if (interval >= 1) return { amount: interval, unit: $t("time_month") }

  interval = Math.floor(seconds / 86400); // days
  if (interval >= 2) return { amount: interval, unit: $t("time_days") }
  if (interval >= 1) return { amount: interval, unit: $t("time_day") }

  interval = Math.floor(seconds / 3600); // hours
  if (interval >= 2) return { amount: interval, unit: $t("time_hours") }
  if (interval >= 1) return { amount: interval, unit: $t("time_hour") }

  interval = Math.floor(seconds / 60); // minutes
  if (interval >= 2) return { amount: interval, unit: $t("time_minutes") }
  if (interval >= 1) return { amount: interval, unit: $t("time_minute") }

  // seconds
  if (seconds >= 2) return { amount: seconds, unit: $t("time_seconds") }
  if (seconds >= 1) return { amount: seconds, unit: $t("time_second") }


  return undefined;
}
export function getTimeAgoString(date?: Date | string): string {
  if (!date) return "";
  if (typeof date === "string") date = new Date(date);
  const time = timeSince(date);
  if (!time) return $t("time_just_now");
  return $t("time_ago", { ...time })
}
